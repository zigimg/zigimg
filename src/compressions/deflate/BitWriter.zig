...
pub fn setWriter(self: *BitWriter, new_writer: *std.Io.Writer) void {
    //assert(self.bits == 0 and self.nbits == 0 and self.nbytes == 0);
    self.inner_writer = new_writer;
}

pub fn flush(self: *BitWriter) Error!void {
    var n = self.nbytes;
    while (self.nbits != 0) {
...
pub fn writeBits(self: *BitWriter, b: u32, nb: u32) Error!void {
...
pub fn writeBytes(self: *BitWriter, bytes: []const u8) Error!void {
    var n = self.nbytes;
    if (self.nbits & 7 != 0) {
