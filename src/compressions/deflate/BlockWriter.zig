...
pub fn init(writer: *std.Io.Writer) BlockWriter {
    return .{
...
pub fn flush(self: *BlockWriter) Error!void {
    try self.bit_writer.flush();
}

pub fn setWriter(self: *BlockWriter, new_writer: *std.Io.Writer) void {
    self.bit_writer.setWriter(new_writer);
}

fn writeCode(self: *BlockWriter, c: hc.HuffCode) Error!void {
...
fn storedHeader(self: *BlockWriter, length: usize, eof: bool) Error!void {
    assert(length <= 65535);
...
fn fixedHeader(self: *BlockWriter, eof: bool) Error!void {
...
pub fn write(self: *BlockWriter, tokens: []const Token, eof: bool, input: ?[]const u8) Error!void {
...
pub fn storedBlock(self: *BlockWriter, input: []const u8, eof: bool) Error!void {
...
pub fn huffmanBlock(self: *BlockWriter, input: []const u8, eof: bool) Error!void {
